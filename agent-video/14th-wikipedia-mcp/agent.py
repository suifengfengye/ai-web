...
def _load_mcp_tools():
    ...
    async def _load():
        required_client = MultiServerMCPClient(
            {
                "wikipedia": {
        ...
        tools = list(await required_client.get_tools())
        try:
            juhe_client = MultiServerMCPClient(
                {
                    "juhe_mcp": {
                        "transport": "sse",
            ...
            juhe_tools = await asyncio.wait_for(juhe_client.get_tools(), timeout=8)
            tools.extend(juhe_tools)
        except Exception:
            print("juhe_mcp 加载失败，已自动降级为仅使用本地 MCP 工具。")
        return tools
