...
def _get_user_agent() -> str:
    return os.getenv("WIKIMEDIA_USER_AGENT") or "DXH_APP/1.0 (contact: https://space.bilibili.com/189613928)"
...
def _search_wikipedia_mediawiki(query: str, language_code: str, limit: int, headers: dict[str, str]) -> str:
...
def search_wikipedia_raw(query: str, language_code: str = "zh", limit: int = 3) -> str:
    ...
    try:
        ...
        if resp.status_code == 403:
            return _search_wikipedia_mediawiki(query, language_code, limit, headers)
        resp.raise_for_status()
        data = resp.json() if resp.content else {}
...
def search_wikipedia(query: str, language_code: str = "zh", limit: int = 3) -> str:
    """在 Wikipedia 中搜索相关词条并返回最匹配词条的完整内容。"""
    return search_wikipedia_raw(query=query, language_code=language_code, limit=limit)
