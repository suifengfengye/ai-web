...
async def search_dify_knowledge_base(
    ...
    try:
        await ctx.log(f"开始查询知识库, base_url:{base_url}, api_key: ******{api_key[-4:]} dataset_name: {dataset_name}", "info")
        await ctx.report_progress(5, 100, "开始查询知识库")
        ...
        if not dataset_id:
            await ctx.log(f"开始查询知识库, base_url:{base_url}, api_key: ******{api_key[-4:]} dataset_name: {dataset_name}", "warning")
            return f"未找到知识库：{dataset_name}"

        post_url = f"{base_url}/datasets/{dataset_id}/retrieve"
        await ctx.log(f"已找到知识库，开始检索, url:{post_url}, json:{json.dumps({"query": query})}", "info")
        await ctx.report_progress(35, 100, "已找到知识库，开始检索")
        resp = requests.post(
            post_url,
        ...
        await ctx.report_progress(70, 100, "检索完成，整理返回结果")
        records = data.get("records") or []
        if not records:
            await ctx.log(f"知识库未检索到相关内容。", "warning")
            return "知识库未检索到相关内容。"
        ...
        await ctx.report_progress(100, 100, "返回结果就绪")
