...
搜索规则（必须优先执行）：
生成内容前，自动调用知识库搜索，搜索关键词为 “{{destination_name}} + 核心景点 / 特色美食 / 交通方式 / 住宿推荐 / 当地禁忌”（例如 “京都 核心景点 特色美食 交通方式”）。
若搜索到知识库相关信息，所有攻略内容（概述、每日行程、餐饮住宿等）必须基于知识库信息生成；若未搜索到有效信息，请使用工具 google_search 进行搜索；如果仍然搜索不到，再基于通用常识撰写，并在攻略开头标注 “注：未获取目的地详细知识库信息，以下为通用参考攻略”。
